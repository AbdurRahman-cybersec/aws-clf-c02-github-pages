(function () {
  const QUESTIONS = Array.isArray(window.PRACTICE_EXAM_4_QUESTIONS)
    ? window.PRACTICE_EXAM_4_QUESTIONS
    : [];
  if (!QUESTIONS.length) return;
  const STORAGE_KEY = "aws-clf-c02-practice-exam-4-markdown-v1";
  const EXAM_SECONDS = 273 * 60;
  ...
  const startPanel = byId("practice4StartPanel");
  const examPanel = byId("practice4ExamPanel");
  const resultsPanel = byId("practice4ResultsPanel");
  const questionCard = byId("practice4QuestionCard");
  const questionGrid = byId("practice4QuestionGrid");
  ...
  function updateResumeButton() {
    const button = byId("resumePracticeExam4");
    if (!button) return;
    try {
  ...
  function updateTimer() {
    const timer = byId("practice4Timer");
    if (!timer) return;
  ...
  function renderQuestion() {
    ...
    byId("practice4QuestionCounter").textContent =
      "Question " + (state.currentPosition + 1) + " of " + state.order.length;

    const domainLabel = byId("practice4DomainLabel");
    ...
    byId("practice4AnsweredCounter").textContent =
      answeredCount + " answered";

    const reviewButton = byId("practice4MarkReview");
    ...
    byId("practice4Previous").disabled = state.currentPosition === 0;
    byId("practice4Next").textContent =
      state.currentPosition === state.order.length - 1
        ? "Return to first →"
        : "Next →";

    const checkButton = byId("practice4CheckAnswer");
  ...
  function startExam(resume) {
    if (!resume) {
      ...
      state.mode = byId("practice4Mode").value;
      state.selectedDomain = byId("practice4DomainFilter").value;
      state.timerEnabled = byId("practice4TimerEnabled").checked;
      state.shuffleEnabled = byId("practice4ShuffleEnabled").checked;
    ...
    resultsPanel.innerHTML = `
      <div class="practice-results-head">
        ...
        <div class="practice-result-message">
          ...
          <p>
            Questions and correct-answer letters come directly from practice-exam-4.md.
          </p>
        </div>
      </div>
  ...
  byId("startPracticeExam4").addEventListener("click", function () {
    startExam(false);
  });

  byId("resumePracticeExam4").addEventListener("click", function () {
    if (loadState()) {
  ...
  byId("clearPracticeProgress4").addEventListener("click", function () {
    if (window.confirm("Clear the saved Quizlet Practice Exam attempt?")) {
      clearState();
    }
  });

  byId("practice4Previous").addEventListener("click", function () {
    state.currentPosition = Math.max(0, state.currentPosition - 1);
    saveState();
    renderQuestion();
  });

  byId("practice4Next").addEventListener("click", function () {
  ...
  byId("practice4MarkReview").addEventListener("click", function () {
  ...
  byId("practice4CheckAnswer").addEventListener("click", checkCurrentAnswer);
  byId("practice4Submit").addEventListener("click", function () {
    submitExam(false);
  });

  byId("practice4ExitButton").addEventListener("click", function () {
    saveState();
    stopTimer();
    examPanel.hidden = true;
    startPanel.hidden = false;
  ...
  document.addEventListener("documentpagechange", function (event) {
    if (event.detail.page !== "practice4") {
      stopTimer();
